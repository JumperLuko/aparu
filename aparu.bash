...
_arch_incomp2() {
  local r="[[:space:]](${1#* }[[:space:]]|[[:alnum:]_]*${1% *})|[[:space:]]-(-${1#* }[[:space:]]|[[:alnum:]_]*${1% *})"
  [[ $COMP_LINE =~ $r ]]
}
...
_paru() {
  ...
  core=('database files help query remove sync upgrade version' 'D F Q R S U V h' 'install remove update upgrade')
  ...
  for o in 'D database' 'F files' 'Q query' 'R remove' 'S sync' 'U upgrade' 'P show' 'G getpkgbuild' 'C chrootctl' 'L repoctl' 'I install'; do
    _arch_incomp2 "$o" && break
  done
